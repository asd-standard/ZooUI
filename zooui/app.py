...
"""ZooUI application entry point — importable from pip install or direct launch."""
...
# ── system dependency check (runs before any heavy imports) ──────────────────


def _check_system_deps() -> None:
    """Check that required system libraries and binaries are available.

    libvips (the C library) is required — prints install instructions and
    exits if missing or unloadable.  pdftoppm is optional — prints a warning.
    """
    MISSING_LIBVIPS_MSG = """\
Missing system dependency: libvips (the C library required by pyvips).
...
Install with:
  ...
  Conda (use conda-forge for all packages):
    conda install -c conda-forge libvips pyvips

  macOS (Homebrew):
    brew install vips

If libvips is installed but still fails, your environment may have
incompatible library versions (e.g., mixing pkgs/main with conda-forge).
Reinstall pyvips from conda-forge:
    conda install -c conda-forge pyvips
"""

    libvips_found = False
    try:
        lib = ctypes.util.find_library("vips") or "libvips.so.42"
        ctypes.CDLL(lib)
        libvips_found = True
    except Exception:
        pass
    if not libvips_found:
        sys.exit(MISSING_LIBVIPS_MSG)

    if not shutil.which("pdftoppm"):
        print(
            "Warning: pdftoppm not found — PDF support will be unavailable.\n"
            "         Install poppler-utils (Debian/Ubuntu) or poppler (conda/Fedora/Homebrew).",
            file=sys.stderr,
        )
...
# ── application imports (safe — system deps verified above) ─────────────────

from PySide6 import QtGui, QtWidgets  # noqa: E402

import zooui.tilesystem.tilemanager as TileManager  # noqa: E402
from zooui.config import ConfigManager, ValidationError  # noqa: E402
from zooui.logger import LoggerConfig, get_logger  # noqa: E402
from zooui.utils._packaging import data_dir  # noqa: E402
...
def main():
    ...
    # Resolve icon path using the packaging utility (works in source, pip, and frozen modes)
