...
"""Step: Right-click string to open modify dialog with Ctrl+Enter accept."""
...
def run(ctx: GUITestContext) -> None:
    ctx.log.section("MOUSE RIGHT-CLICK - STRING MODIFICATION (Ctrl+Enter ACCEPT)")
    ensure_test_scene_loaded(ctx)
    zui = ctx.window.zui
    w, h = zui.width(), zui.height()
    string_center = QPoint(int(w * 0.5), int(h * 0.85))
    ctx.log.action("Right-click on test string, accepting modify dialog with Ctrl+Enter")
    accept_open_dialog_via_ctrl_enter(ctx)
    simulate_mouse_click(ctx, string_center, Qt.RightButton)
    wait(ctx, DEFAULT_DELAY_MS, "Observe: String modification dialog accepted via Ctrl+Enter")
    ctx.log.success("String modification dialog accepted with Ctrl+Enter")
